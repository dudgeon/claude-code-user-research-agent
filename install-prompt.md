...
3. **End-of-session logging**: When the user's task appears complete, after a substantial commit, or when the user explicitly ends the session, suggest generating a research log via the `research-log` skill. If the user declines, skip it gracefully — never nag.
